...
import EditActivationFormEx from './EditActivationFormEx'; 

const noneDisplayI = 'noneDisplayI';
const displayI = 'displayI';
      
  
const AdminPostsArabic = ({loading ,match, addLike,auth,five_stars,
  removeLike, 
  deletePost,
  ...
  // const [getfivestarsuser,setgetfivestarsuser]= useState([])
    ...
    useEffect(() => {
      ...
      .catch((err) => {
        ...
        axios.get('/api/posts/activationCompleteAdmin')
        .then(res => {
          //console.log(res);
        ...
        axios.get('/api/posts/activationNoAdmin')
        .then(res => {
          //console.log(res);
     ...
     const showModal1 = async e => {
      ...
      <center>
        ...
        <Link to="/ar/dashboard/posts/searchAdminPost" className="Action-button-add-ads">  <i className="fa fa-search fa-1x"></i> بحثك </Link>
        </center>
...
{/* //premium  ----------------------------------*/}
  ...
  <EditActivationFormEx postId={post._id} validity={post.activation} />
    ...
    return(
      ...
      {user.validity === "super" || user.validity === "admin"  ?  AdminPostsArabic : notValidPage}
      </div>
      </div>
      );
}
